...
334 if(dat_readerr) then
       print *, 'Error when reading the file '//trim(adjustl(line)); stop
    end if
...
if(fs_size_L0>fs_size_DI) then
  print *, "Error: fs_size_L0 must be smaller than fs_size_DI ! "; stop
end if
...
if(trim(adjustl(method_so))=='L1L0' .and. fs_size_DI/=fs_size_L0 .and. ptype==1) then
    ...
    do ntry=1,L1_nlambda
        ...
        if(k/=nf .and. mpirank==0 ) then
          print *, 'LASSO: ID of selected features not correctly identified !'; stop
        end if
...
! collecting the best models
   ...
   if(mpirank==0) then
      ...
      write(line_name,'(a,i4.4,a,i3.3)') 'top',min(int8(nmodels),totalm),'_D',idimen
...
2008  format(i12,2f12.6,a,*(i8))
      ...
      write(line_name,'(a,i4.4,a,i3.3,a)') 'top',min(int8(nmodels),totalm),'_D',idimen,'_coeff'
      open(fileunit,file='models/'//trim(adjustl(line_name)),status='replace')
...
write(9,'(i3,a)') idimen,'D descriptor: '

do i=1,idimen
write(9,'(a,i3.3,3a,i6.6)') '     d',i,' = ',trim(adjustl(fname(id(i)))),'    feature_ID:',id(i)
end do

write(9,'(/,i3,a)') idimen, 'D model(y=sum(ci*di)+c0): '  
...
do i=1,ntask
  write(9,2009) '     coeff.(ci)_task',i,': ', betamin(:idimen,i)
  write(9,'(a20,i3.3,a2,e20.10)') '             c0_task',i,': ', interceptmin(i)
  write(9,'(a20,i3.3,a2,2e20.10)') '     RMSE,MaxAE_task',i,': ', rmse(i),maxae(i)
  write(line_name,'(a,i3.3,a,i3.3,a)') 'desc_D',idimen,'_t',i,'.dat'
...
if(task_weighting==1) then  ! tasks are treated equally
   write(9,'(/a,2f10.6)')  '     RMSE and MaxAE of the model: ',sqrt(sum(rmse**2)/ntask),maxval(maxae)
else if(task_weighting==2) then  ! weighted by sample size
   if(nreaction==0) then
     write(9,'(/a,2f10.6)')  '     RMSE and MaxAE of the model: ',sqrt(sum((rmse**2)*nsample)/sum(nsample)),maxval(maxae)
   elseif(nreaction>0) then
     write(9,'(/a,2f10.6)')  '     RMSE and MaxAE of the model: ',rmse,maxval(maxae)
   endif
end if
...
real*8  x(:,:,:),mscore(max(nmodels,1),2),overlap_size,overlap_size_tmp,select_overlap_size(max(nmodels,1)),&
...
ndata_ol=0
do itask=1,ntask
  write(line_name,'(a,i3.3,a,i3.3,a)') 'desc_D',idimen,'_t',itask,'.dat'
  open(fileunit,file='desc_dat/'//trim(adjustl(line_name)),status='replace')
...
if(idimen<=2 .and. convexpair) then
  ...
  write(9,'(a)') 'Note: negative means no overlap, with the magnitude being the min distance between domains.'
end if
