...
if(trim(adjustl(task))=='FCDI') then
  dim_DI=iFCDI
else
  dim_DI=desc_dim
end if

if(mpirank==0 .and. trim(adjustl(task))/='FCDI') then
...
!--------------------------
allocate(xinput(maxns,fs_size_DI,nprop))
allocate(yinput(maxns,nprop))
allocate(fname(fs_size_DI))
allocate(weight(maxns,nprop))
...
do i=1,nprop
  ...
  open(funit,file='feature_space/'//trim(adjustl(line)),status='old')
   ...
   call mpi_bcast(yinput,maxns*nprop,mpi_double_precision,0,mpi_comm_world,mpierr)
   call mpi_bcast(xinput,maxns*fs_size_DI*nprop,mpi_double_precision,0,mpi_comm_world,mpierr)
...
weight=1.0
if(L1_weighted) then
open(funit,file='prop.weight',status='old')
...
if(trim(adjustl(method))=='L1L0' .and. fs_size_DI/=fs_size_L0 .and. trim(adjustl(ptype))=='quanti') then
    ...
    ! L1 of the L1L0
    !----------------
    allocate(xprime(maxns,fs_size_DI,nprop))
    allocate(xdprime(maxns,fs_size_DI,nprop))
    allocate(xmean(fs_size_DI,nprop))
    allocate(norm_xprime(fs_size_DI,nprop))
    allocate(prod_xtx(fs_size_DI,ncol(mpirank+1),nprop))
    allocate(intercept(nprop))
    allocate(beta(fs_size_DI,nprop)) 
    allocate(beta_init(fs_size_DI,nprop))
    allocate(yprime(maxns,nprop))
    allocate(prod_xty(fs_size_DI,nprop))
    allocate(lassormse(nprop))
    allocate(ymean(nprop))
    ...
    do ntry=1,L1_nlambda
        ...
        totalrmse=sqrt(sum(lassormse**2)/nprop)  ! quadratic mean of lassormses
...
integer nactive,activeset(:),i,j,k,l,loc(1),ii(dim_DI),idimen,select_model(max(nm_output,1),dim_DI),&
        mID(max(nm_output,1),dim_DI)!,random(maxval(nsample),nprop,CV_repeat)
integer*8 totalm,bigm,nall,nrecord,nupper,nlower
real*8  x(:,:,:),y(:,:),tmp,tmp_LS,tmp2_LS,yfit,wrmse(nprop),LSrmse(nprop),LSmaxae(nprop),beta(dim_DI,nprop),&
intercept(nprop),select_LS(max(nm_output,1)),select_LSmaxae(max(nm_output,1)),&
mscore(max(nm_output,1),2),select_metric(max(nm_output,1))
!select_CV(max(nm_output,1)),select_CVmaxae(max(nm_output,1)),tmp_CV,tmp2_CV,CVrmse(nprop),CVmaxae(nprop)
character fname(:)*200,line_name*100
logical isgood
integer*8 njob(mpisize)
real*8 mpicollect(mpisize)
real   progress
! model scores: LS_RMSE,LS_MaxAE,CV_RMSE,CV_MaxAE
...
do idimen=1,dim_DI
   if(idimen<dim_DI .and. trim(adjustl(task))=='FCDI') cycle
...
! get the best nm_output models on each CPU core
   ...
   do k=1,nactive   ! features
      ...
      if(float(nrecord-nlower+1)/float(njob(mpirank+1))>=progress) then
        write(*,'(a,i4,a,f6.1,a)')  'mpirank= ',mpirank,'    progress =',progress*100,'%'
        progress=progress+0.2
      end if

      !  LS and CV of this model
      if ( trim(adjustl(metric))=='LS_RMSE' .or. trim(adjustl(metric))=='LS_MaxAE') then
         do i=1,nprop
         ...
         tmp_LS=sqrt(sum(LSrmse**2)/nprop)  !overall error
         tmp2_LS=sqrt(sum(LSmaxae**2)/nprop)         ! overall maxAE
    ...
    !      end do
    !     end do
    !     tmp_CV=sqrt(sum(CVrmse)/CV_repeat/nprop)  ! quadratic mean
    !     tmp2_CV=sqrt(sum(CVmaxae)/CV_repeat/nprop)
      end if
    
      isgood=.false.
      if ( trim(adjustl(metric))=='LS_RMSE' .and. any(tmp_LS <select_LS)) then
         isgood = .true.
         loc=maxloc(select_LS)
      else if ( trim(adjustl(metric))=='LS_MaxAE' .and. any(tmp2_ls <select_LSmaxae)) then
         isgood = .true.
...
! calculate all errors of the nm_output model
   do k=1,min(int8(nm_output),totalm)
      if ( trim(adjustl(metric))/='LS_RMSE' .and. trim(adjustl(metric))/='LS_MaxAE') then
         do i=1,nprop
        call orth_de(x(:nsample(i),[select_model(k,:idimen)],i),y(:nsample(i),i),intercept(i),beta(:idimen,i),LSrmse(i))
         ...
         select_LS(k)=sqrt(sum(LSrmse**2)/nprop)  !overall error: quadratic mean
         select_LSmaxae(k)=sqrt(sum(LSmaxae**2)/nprop)         ! overall maxAE
    ...
    !      end do
    !     end do
    !     select_CV(k)=sqrt(sum(CVrmse)/CV_repeat/nprop)  ! overall CV RMSE
    !     select_CVmaxae(k)=sqrt(sum(CVmaxae)/CV_repeat/nprop)
      end if
   end do
...
! select_model,select_LS,select_LSmaxae,select_CV,select_CVmaxae,mID,mscore(RMSE,MaxAE,CV_RMSE,CV_MaxAE)
   ...
   else if (trim(adjustl(metric))=='LS_MaxAE') then
...
!      call writeout(idimen,mID(1,:idimen),LSrmse,LSmaxae,beta,intercept,x,y,fname,CVrmse,CVmaxae)
      ...
      write(funit,'(4a12)') 'Rank','LS_RMSE','LS_MaxAE','Feature_ID'
...
if(idimen==desc_dim .and. trim(adjustl(task))=='FCDI') then
  write(9,'(/a)')  'Final model/descriptor to report'
else if(idimen<desc_dim .and. trim(adjustl(task))=='FCDI') then
  write(9,'(/a)')  'Model/descriptor for generating residual:'
else if(trim(adjustl(task))=='DI') then
  write(9,'(/a)')  'Model/descriptor to report'
end if
write(9,'(a)') '============================================='
write(9,'(i3,a)') idimen,'D descriptor (model): '
write(9,'(a,2f10.6)')  'Total LS_RMSE,LS_MaxAE: ',sqrt(sum(LSrmse**2)/nprop),sqrt(sum(LSmaxae**2)/nprop) 
!,sqrt(sum(CVrmse**2)/nprop),sqrt(sum(CVmaxae**2)/nprop)
...
2013  format(i10,i20,f20.5,a,*(i8))
      ...
      if(idimen==2) then
         ...
         do i=1,ngroup(itask,1000)
           ...
           write(funit,'(a,i5,g20.10)') 'Hull',i,convex2d_area(hull(:j,:))
           do k=1,j
...
integer idimen,i,j,id(:),k,l,nh,itask,mm1,mm2,mm3,mm4,realn_ol
...
do itask=1,nprop
  ...
  do j=sum(ngroup(itask,:i-1))+1,sum(ngroup(itask,:i))  ! samples in this group
     ...
     do k=1,ngroup(itask,1000)  ! between sample j and group k other than i
       ...
       if(idimen==1) then
               if(x(j,id(idimen),itask)>=minval(x(mm1:mm2,id(idimen),itask)) .and. &
                        x(j,id(idimen),itask)<=maxval(x(mm1:mm2,id(idimen),itask)) ) then
                 inside=.true.
                 realn_ol=realn_ol+1
                 exit
               end if
