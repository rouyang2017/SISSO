...
! one-off FC: output space.name, space_pxxx.dat
! FC for FCDI: output space_xxd.name, space_xxd_pxxx.dat
!----------------------------------------------------------------------
...
integer*8 aaa,i,j,k,l,ll,mm1,mm2,nf(maxcomb),nout_sis,mpii,mpij,mpik
...
if(nvf>0) then
  ...
  do i=1,nvf   ! linking vectors features
    ...
    feat_in1(2,nsf+i)=abs(sum(tag*sum(vfeat(:,:,i),2)))/nprop ! vf tag to the 2nd element
  end do
end if
...
elseif( trim(adjustl(task))=='FCDI') then
...
do j=1,nprop
 ...
 if(mpirank==0 .and. trim(adjustl(ptype))=='quanti' .and. &
    (trim(adjustl(task))=='FC' .or. (trim(adjustl(task))=='FCDI' .and. iFCDI==1)) )&
  ...
  IF (foutsave) THEN
       ...
       do i=1,nf(icomb)
         if( isscalar(name_out(i)) ) then
          ftag(i)=sum(tag*fout(:,i))/nprop
         else
          ftag(i)=fout(2,i)/nprop
         end if
       end do
...
call iter_sis_p(nfpcore_this,available,ftag_select,name_select,f_select,nout_sis,sisfeat,name_sisfeat)
...
!   write(9,'(a,"   ",a)') 'The best feature by SIS is: ',trim(name_sisfeat(1))
   ...
   if(trim(adjustl(ptype))=='quanti') then
      do i=1,nout_sis
        tmp_score=sis_score(sisfeat(:,i),trainy_c,'centered')
        write(funit,'(2a,f12.4)') trim(name_sisfeat(i)),'  corr=',tmp_score(1)
      end do
   else if(trim(adjustl(ptype))=='quali') then
      do i=1,nout_sis
        tmp_score=sis_score(sisfeat(:,i),trainy,'centered') ! for classification
   ...
   do j=1,nprop
      ...
      mm2=sum(nsample(:j))
      if(trim(adjustl(task))=='FCDI') then
        write(line,'(a,i3.3,a,i3.3,a)') 'space_',iFCDI,'d_p',k,'.dat'
      else
        write(line,'(a,i3.3,a)') 'space_p',k,'.dat'
      end if
      open(funit,file='feature_space/'//trim(adjustl(line)),status='replace')
      if(trim(adjustl(ptype))=='quanti') then
2001  format(*(e15.5))
        do ll=mm1,mm2
          write(funit,2001) trainy(ll),sisfeat(ll,:nout_sis)
        end do
      else
        do ll=mm1,mm2
          write(funit,2001) sisfeat(ll,:nout_sis)  ! no y value for qualitative
        end do
      end if
      close(funit)
   end do

   write(*,'(3a,i10)') 'Size of the SIS-selected subspace from ',phiname,': ',nout_sis
   if(nout_sis<subs_sis) then
   write(*,'(a)') 'WARNING: the actual size of the selected subspace is smaller than that specified in "SISSO.in" !!!'
   write(*,'(a)') 'Please either reduce the "subs_sis" in "SISSO.in" or manually do FC followed by DI with a proper &
                   setting of the "fs_size_DI" and "fs_size_L0".'
   write(*,'(/)')
   end if
   write(9,'(3a,i10)') 'Size of the SIS-selected subspace from ',phiname,': ',nout_sis
end if  
...
!sure independence screening
if(trim(adjustl(ptype))=='quanti') scoretmp=sis_score(feat,trainy_c,'centered')
if(trim(adjustl(ptype))=='quali')   scoretmp=sis_score(feat,trainy,'centered') !classification
...
ftag_select(nselect)=abs(sum(tag*feat))/nprop   ! positive for ftag_select; positive & negative for ftag
...
subroutine iter_sis_p(nfpcore_this,available,ftag,fname,feat,nout_sis,sisfeat,name_sisfeat)
! parallel sure independence screening

real*8 ftag(:),tmp,sisfeat(:,:),pscore(mpisize,2),pftag(mpisize),feat(:,:)
real*8,allocatable:: score(:,:)
integer*8 i,j,k,l,ll,mpii,mpij,nfpcore_this(:),nout_sis,loc(2)
...
do while(i<subs_sis .and. any(pavailable))
     ...
     if(trim(adjustl(ptype))=='quanti') then
        do k=1,nfpcore_this(mpirank+1)
          if(available(k)) score(k,:)=sis_score(feat(:,k),trainy_c,'centered')
        end do
     else if(trim(adjustl(ptype))=='quali') then
        do k=1,nfpcore_this(mpirank+1)
          if(available(k)) score(k,:)=sis_score(feat(:,k),trainy,'centered') ! for classification
        end do
     end if
...
function sis_score(feat,y,center)   
...
integer i,j,mm1,mm2,mm3,mm4,k,kk,l,overlap_n,nf1,nf2,itask
character(len=*) center
real*8 feat(:),sdfeat(ubound(feat,1)),tmp(nprop),sis_score(2),y(:),&
sdy(ubound(y,1)),xnorm(nprop),xmean(nprop),overlap_length,length_tmp,&
...
if(trim(adjustl(center))=='centered') then
   ...
   mm1=sum(nsample(:j-1))+1
   mm2=sum(nsample(:j))
   sdy(mm1:mm2)=y(mm1:mm2)-sum(y(mm1:mm2))/(mm2-mm1+1)
   end do
end if
...
if(trim(adjustl(ptype))=='quanti') then
   do j=1,nprop
       ...
       tmp(j)=abs(sum(sdfeat(mm1:mm2)*sdy(mm1:mm2)))   ! |xy| absolute inner product
   end do
  ! score ranges from 0 to 1
  sis_score(1)=sqrt(sum(tmp**2)/nprop)  ! quadratic mean of the scores of different tasks
  sis_score(1)=sis_score(1)/sqrt(sum(sdy**2)/nprop)  ! normalize the total score
  sis_score(2)=1.d0   ! not used
   ...
   ! calculate overlap between domains of property i
        do i=1,ngroup(itask,1000)-1   ! ngroup(itask,1000) record the number of groups in this task
            ...
            nf1=0
            do k=mm1,mm2
               if(sdy(k)<0.5) then  ! y=1: classified, y=0: unclassified
                   nf1=nf1+1
...
if(goodf(feat2,name_feat,dimens,comp)) then
  ...
  fout(1,nf)=nvf_new
  fout(2,nf)=abs(sum(tag*feat2))/nprop
  vfeat(:,:,nvf_new)=feat
  name_out(nf)=name_feat
  lastop_out(nf)=lastop
  complexity_out(nf)=comp
