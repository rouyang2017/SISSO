...
elseif( trim(adjustl(calc))=='FCDI') then
...
!sure independence screening
if(trim(adjustl(ptype))=='quanti') scoretmp=sis_score(feat,trainy_c)
if(trim(adjustl(ptype))=='quali')   scoretmp=sis_score(feat,trainy) !classification
...
if(trim(adjustl(ptype))=='quanti') then   ! 'quanti' yyy are already moved to center
   do j=1,ntask
       mm1=sum(nsample(:j-1))+1
       mm2=sum(nsample(:j))
