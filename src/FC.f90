...
IF (iFCDI>1 .and. nsis(iFCDI-1)<nf_sis(iFCDI-1)) THEN
   if(mpirank==0) then
      write(*,'(a)') 'Warning: Size of the last selected subspace < nf_sis! No more FC will be performed.' 
      write(9,'(a)') 'Wrning: Size of the last selected subspace < nf_sis! No more FC will be performed.'
   end if
   return
end if
...
! A 'tag' vector to ensure every different feature unique in terms of their data
do j=1,ntask
do i=1,nsample(j)    
...
if(nvf>0) then  ! there are vectors features
  vfeat(:,:,:nvf)=pvfeat(:,:,:nvf)  ! vector features stored in vfeat
  do i=1,nvf  
    feat_in1(1,nsf+i)=i    ! vf ID stored in the 1st element
!    feat_in1(2,nsf+i)=sqrt(sum((tag+sum(vfeat(:,:,i),2))**2))  ! norm of the new vector
  end do
end if

! avoid reselecting the same features
...
if(nreaction==0) then  ! not yet public
   do j=1,ntask
    ...
    if(mpirank==0 .and. ptype==1 .and. iFCDI==1) &
        write(9,2000) 'Population Standard Deviation (SD) of the task ',j,': ',SD(j)
   end do
...
do icomb=1,rung
    ...
    ! divide the task into n parts with equal workload
    ! X1, X2, ..., Xi, ...
    ...
    ! mpin: number of features for each core, mpin2: starting id
      mpin=0
      mpin2=0
     ...
     ! balance the workload between CPU cores
      do i=2,mpisize,2
         j=mpisize-i+2
         if(j<=i) exit
         mpii=mpin(i)
  ...
  IF (foutsave) THEN
       ...
       ! ensure every different feature unique in terms of their data for redundant check
       ...
       !----------------------------------------------------------------------------------------
       ! redundant check inside each core: output the "order" and "available".
       if(mpirank==0) then
       ...
       ! redundant check between different cores,output the "available"
      ...
      !--------------------------------------------------------------------
      ! cp results from _out to _in1 in mpirank0
      !--------------------------------------------------------------------
       i=nf(icomb)
        ...
        !---------------------------------
         if(nvf>0) then
            if(mpirank/=0) then
              ...
              if(sum(nvfpcore)>ubound(vfeat,3)) call addm_vf(sum(nvfpcore)-ubound(vfeat,3)) ! vector features
              do mpii=1,mpisize-1
...
! name_select: name/expressions of the selected features on each core
...
if(mpirank==0) then
   ...
   write(line,'(a,i3.3,a)') 'space_D',iFCDI,'.expressions'
   ...
   ! output the expression/feature data
2001  format(*(e20.10))
   if(nreaction==0) then
     do j=1,ntask
        mm1=sum(nsample(:j-1))+1
        mm2=sum(nsample(:j))
        write(line,'(a,i3.3,a,i3.3,a)') 'space_D',iFCDI,'_t',j,'.dat'
        open(fileunit,file='SIS_subspaces/'//trim(adjustl(line)),status='replace')
        if(ptype==1) then
          do ll=mm1,mm2
   ...
   elseif(nreaction>0) then
        write(line,'(a,i3.3,a,i3.3,a)') 'space_D',iFCDI,'_t',1,'.dat'
        open(fileunit,file='SIS_subspaces/'//trim(adjustl(line)),status='replace')
        do ll=1,nreaction
          write(fileunit,2001) trainy(ll),sisfeat(ll,:nsis(iFCDI))
        end do
   ...
   write(*,'(a)') 'Warning: Size of the selected subspace is smaller than nf_sis (SISSO.in) !!!'
   end if
   write(9,'(3a,i10)') 'Size of the SIS-selected subspace from ',phiname,': ',nsis(iFCDI)
   if(any(usedup_warning>0) .and. ffdecorr ) &
     write(9,'(/a,i5,a/)') 'Warning: not enough features for SIS. Please increase decorr_alpha and rerun SISSO'
end if  
...
! unary operators
! element-wise operation for vector features
      counter=counter+1
       ...
       IF(index(op,'(+)')/=0 .or. index(op,'(-)')/=0  .or. index(op,'(|-|)')/=0   ) THEN
               ...
               IF(index(op,'(+)')/=0) then
                ...
                IF(index(op,'(|-|)')/=0) then  ! absolute difference
                  ...
                  if( isscalar(name_in1(i)) .and. isscalar(name_in2(j))  ) then
                       if(index(op,'(-)')/=0) then
                         tmp=abs(tmp)   ! if (-) was just applied previously, simply take the abs
                       else
...
! not to be selected but can be used for further combination
...
if(nvf>0) write(*,'(a,i15)') 'Total Number of vector features: ',j
         ...
         if(mpirank==0 .and. i>1) then
            ...
            if(abs(score_sisfeat(j,1)-score_sisfeat(i,1))>dd1 .or. &
              (abs(score_sisfeat(j,1)-score_sisfeat(i,1))<=dd1 .and. &
               abs(score_sisfeat(j,2)-score_sisfeat(i,2))>dd1) )  exit ! if not with similar scores
...
ELSE
  ...
  ! sort the features 'fidentity' from large to small
  order(1)=1
  n=1
  do i=2,nf
...
if(ptype==1) then  
   ...
   elseif(nreaction>0) then
     ...
     if(xnorm(1)>1d-50) then
        sdfeat(:nreaction)=sdfeat(:nreaction)/xnorm(1)
...
! transforming a vector feature to a scalar feature
do i=1,d1
