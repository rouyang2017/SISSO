...
function convex3d_in(set,point,width,ntri,triangles)
! check if a point is inside a 3d convex hull
! input: the full data set, the point, the boundary tolerence, and the hull-triangles
! output: .false. or .true.

integer i,j,k,ntri,ninter,triangles(:,:)
real*8 point(3),point_b(3),set(:,:),width,pproj(3),pinter(3),v1(3),v2(3),v3(3),normal(3),ddd,ttt,area,dist(3),vtmp(3,3)
...
ninter=0
point_b=(/point(1)+1.0,point(2)+1.0,point(3)+1.0/) ! point-point_b(arbitrary) is the line
do i=1,ntri
  v1=set(triangles(i,2),:)-set(triangles(i,1),:)
  v2=set(triangles(i,3),:)-set(triangles(i,1),:)
  v3=set(triangles(i,3),:)-set(triangles(i,2),:)
  normal=crosspro(v1,v2)  ! normal of the triangle plane
  ddd=-sum(normal*set(triangles(i,1),:)) ! the d of plane: ax+by+cz+d=0
  ttt=-(sum(normal*point)+ddd)/sum(normal**2)  ! the t of line equation
  pproj=point+normal*ttt  ! projection of the point on the triangle plane

  ! measure the distance of the point from the triangle
  if(sqrt(sum((point-pproj)**2))<=width) then
     if(intriangle(pproj,set(triangles(i,1),:),set(triangles(i,2),:),set(triangles(i,3),:))) then !projection inside
        convex3d_in=.true.
        return
     else
        vtmp(1,:)=point-set(triangles(i,1),:)
        vtmp(2,:)=point-set(triangles(i,2),:)
        vtmp(3,:)=point-set(triangles(i,3),:)
  ...
  if(sum((point-point_b)*normal)/=0.d0 ) then ! line-plane is not parallel
    pinter=interlp(set(triangles(i,1),:),set(triangles(i,2),:),set(triangles(i,3),:),point,point_b) 
    if(intriangle(pinter,set(triangles(i,1),:),set(triangles(i,2),:),set(triangles(i,3),:))) then 
        ninter=ninter+1
...
do i=1,ninter-1
  do j=i+1,ninter
    ...
    v1=inter_all(i,:)-point
    v2=inter_all(j,:)-point
    if(sum(v1*v2)<0.d0) then  ! if v1 and v2 point to different direction, the point is inside
       convex3d_in=.true.     ! otherwise the point is outside
       return
    end if
  end do
end do
...
do i=1,nset2
  if(convex3d_in(set1,set2(i,:),width,ntri1,triangles)) no1=no1+1
end do
...
do i=1,nset1
  if(convex3d_in(set2,set1(i,:),width,ntri2,triangles)) no2=no2+1
end do
deallocate(triangles)
