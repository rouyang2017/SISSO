...
if(mpirank==0) then
 ...
 write(9,'(a)') 'Version SISSO.2.4, January 8, 2019.'
 write(9,'(a/)')'================================================================================'
end if
...
!   read(line_para(i+1:),*,err=1001) CV_repeat
   ...
   case('fs_size_DI')
   if(trim(adjustl(calc))=='DI') read(line_para(i+1:),*,err=1001) fs_size_DI
   case('fs_size_L0')
   if(trim(adjustl(calc))=='DI' .or. (trim(adjustl(calc))=='FCDI' .and. trim(adjustl(method))=='L1L0') ) &
...
task_weighting=1          ! for 'quanti'. 1: no weighting (tasks treated equally) 2: weighted by #sample_task_i/total_sample.
...
!   write(9,'(a,i8)')  'number of primary features that must appear in each of the selected features: ',npf_must
   ...
   if(trim(adjustl(ptype))=='quanti') then
     write(9,'(a,a)')  'metric for the DI: ',trim(metric)
