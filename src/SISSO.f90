...
!-------------------
implicit none
integer i,j,k,l,icontinue,iostatus,nl1l0,datalen
...
if(nreaction>0 .and. (ptype==2 .or. ntask>1)) then
 print *, "Error: Reaction ML works only with ptype=1 and ntask=1 !"; stop
end if
...
if(mpirank==0) then
 ...
 write(9,'(a)') '             Version SISSO.3.3, July, 2023.                '
 write(9,'(a/)')'****************************************************************'
end if
...
allocate(feature_units(ndimtype,nsf+nvf)) ! the units of features
call read_para_b 
if(nreaction>0) then
  npoints=nreaction
else
...
allocate(prop_y(npoints))   ! target property
...
allocate(res(npoints))   ! residual errors

datalen=1000+max(nsf,nvf*vfsize)*20 ! maximal line length in train.dat, train_vf.dat, reaction.dat
...
do iFCDI=icontinue,desc_dim
   ...
   if(mpirank==0) write(9,'(a,i10)') 'Total number of SIS-selected features from all dimensions: ',sum(nsis(:iFCDI))
...
if(iFCDI==1) then
  ...
  do k=1,ntask
     i=i+1 
     write(line_short1,'(a,i3.3,a,i3.3,a)') 'desc_D',iFCDI-1,'_t',i,'.dat'
     write(line_short2,'(a,i3.3,a,i3.3,a)') 'res_D',iFCDI-1,'_t',i,'.dat'
...
write(line_short,'(a,i3.3,a)') 'space_D',iFCDI,'.expressions'
...
! Data of the expressions
i=0
do k=1,ntask
    i=i+1
    ! previous subspaces
    if(iFCDI>1) then
        write(line_short,'(a,i3.3,a)') 'Uspace_t',i,'.dat'
    ...
    ! this subspace
    write(line_short,'(a,i3.3,a,i3.3,a)') 'space_D',iFCDI,'_t',i,'.dat'
    open(fileunit,file='SIS_subspaces/'//trim(adjustl(line_short)),status='old')
    if(nreaction==0) then
        if(ptype==1) then
           do l=1,nsample(k)
...
do while(.true.)
   ...
   case('funit')
     ...
     if(index(funit_line(k+1:),'(')>0) ndimtype=0
     k=0
     do while (index(funit_line(k+1:),'(')>0)  ! calculate the ndimtype
...
return
1001 print *, 'Error while reading the file "SISSO.in", at: '//trim(line_short); stop

end subroutine

! read in the data for nsample, funit, ops, and isconvex
...
return
1002 print *, 'Error while reading the file "SISSO.in" for the parameter: nsample'; stop
1003 print *, 'Error while reading the file "SISSO.in" for the parameter: isconvex'; stop
10031 print *, 'Error while reading the file "SISSO.in" for the parameter: ops'; stop
1004 print *, 'Error while reading the file "SISSO.in" for the parameter: funit'//trim(funit_line); stop
...
character(len=lname) string_tmp(2+nsf+nvf),reactionline(100)*10000,samplename(sum(nsample)),line_verylong*datalen
 ...
 open(fileunit,file='train.dat',status='old')
   ...
   do i=1,sum(nsample)
       ...
       if(ptype==1 .and. nreaction==0) then
         read(line_verylong,*,err=1005) prop_y(i),psfeat(i,:)
       else  ! ptype==2 or nreaction>0
         read(line_verylong,*,err=1005) psfeat(i,:)   ! no y values in the train.dat file for classification
 ...
 if(nvf>0) then
   ...
   read(fileunit,'(a)') line_verylong  

   call sepchange(line_verylong)
   call string_split(line_verylong,pfname(nsf+1:),' ')  ! save the vector-feature expressions

   do i=1,sum(nsample)
      read(fileunit,'(a)') line_verylong
      read(line_verylong,*,err=1006) (pvfeat(i,:,j),j=1,nvf)
   end do
   close(fileunit)
 end if
...
if(nreaction>0)then
   ...
   do i=1,nreaction
      ...
      do j=1,(nentry-1)/2
        ...
        read(reactionline(j+(nentry-1)/2),*,err=1007) react_coeff(i,j)
      end do
      read(reactionline(nentry),*,err=1007) prop_y(i)
   end do
   close(fileunit)
 end if
...
1006 print *, 'Error while reading the file "train_vf.dat", at: '//trim(line_verylong); stop
...
decorr_theta=1.0        ! Threshold (<=1) for decorrelation (one of the highly correlated feature to be removed).
decorr_delta=0.0        ! Size of the score-window within which corr. are evaluated (>=0).
decorr_alpha=1.0        ! Increasing the subspace by a factor of alpha, alpha*nf_sis
...
L1_weighted=.false.     ! Weighted observations? (provide file lasso.weight if yes)
nsis=0 
!----------------------
...
2004   format(*(a))
   ...
   if(nvf>0) then
     ...
     write(9,'(a,i8)') 'Size of vector feature: ',vfsize
     write(9,'(a,a)')  'Method for transforming a vector to a scalar? ',trim(vf2sf)
   end if
   ...
   if(ffdecorr) then
      ...
      write(9,'(a,f15.5)') 'Increasing the subspace by a factor of alpha, alpha*nf_sis: ',decorr_alpha
   end if
   ...
   if(ptype==1) then
     ...
     if(trim(adjustl(method_so))=='L1L0') then
       ...
       write(9,'(a,l6)') 'Weighted observations (requires the file lasso.weight)? ',L1_weighted
