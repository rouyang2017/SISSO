...
# by using python3 or new versions:  python3 VarSelect_SISSO.py


############################################ User Input #####################################
# The values below are good for usual computing power. Please make changes for your jobs
# -------------------------------------------------------------------------------------------
n_init = 10           # Initial size of the S in [J. Chem. Theory Comput. 18, 4945 (2022)]
n_RS = 4              # The size of the Sa
n_max =23             # Maximal size of the S (S=Sa+Sb)
...
runSISSO = 'mpirun -np 64  SISSO.3.3 > SISSO.log'  # The command to run SISSO
#############################################################################################
...
def SISSO_out_reader(SISSO_out_folder, dimension, all_features, maths_operators):
    ...
    RMSE_dim = 0
    for i in range(len(SISSO_out_file)):
        if SISSO_out_file[i].count('RMSE and MaxAE of the model:'):
            RMSE_dim += 1
            RMSE_str = SISSO_out_file[i].replace('RMSE and MaxAE of the model:', '').strip()
            RMSE = ''
...
def initial_SISSO_in_2_output_parameter(initial_file_dir, all_features_list, output_parameter):
    ...
    parameter_startwith_dict = {1: "funit", 2: "ops", 3: 'desc_dim'}
...
VS_results.write(
    "Iter. \t %of_visited_var.\tRMSE_of_this_step\tLowest_RMSE\t[var. in the lowest-RMSE-model]\t[var. deselected at this step]\t\n")
VS_results.close()
alpha_dict = {}
