...
!                      line3, highest descriptor dimension in SISSO.out; line4, property type (1:'quanti', 2: 'quali')
...
if(fexist) then
 ...
 read(5,*) ndim
 write(6,'(a)') 'Property type (1: quantitative (continuous), 2: qualitative (categorical): '
 read(5,*) ptype
end if
...
do while(.not. eof(1))
   ...
   if(index(line,'D descriptor ')/=0) then
      ...
      do i=1,ns
        write(4,'(<nd>e15.5)') dat_desc(i,:nd)
      end do
      ...
      if(ptype==1) then
         ...
         write(3,'(3e15.5)') y(i),(intercept+sum(dat_desc(i,:nd)*coeff(:nd))),res(i)
         end do
         rmse=sqrt(sum(res**2)/ns)
         maxae=maxval(abs(res))
         print *,'RMSE and MaxAE: ',rmse,maxae
         write(3,'(a,2f15.5)') 'Prediction RMSE and MaxAE: ',rmse,maxae
      elseif(ptype==2) then
